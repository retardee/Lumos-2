...
/mob/living/carbon/human/update_inv_w_underwear()
	if(!HAS_TRAIT(src, TRAIT_HUMAN_NO_RENDER))
		...
		if(istype(w_underwear, /obj/item/clothing/underwear))
			...
			var/alt_worn = U.mob_overlay_icon || 'modular_lumos/icons/mob/clothing/underwear.dmi'
			var/variant_flag = NONE

			if((DIGITIGRADE in dna.species.species_traits) && U.mutantrace_variation & STYLE_DIGITIGRADE && !(U.mutantrace_variation & STYLE_NO_ANTHRO_ICON))
				alt_worn = U.anthro_mob_worn_overlay || 'modular_lumos/icons/mob/clothing/underwear_digi.dmi'
				variant_flag |= STYLE_DIGITIGRADE
...
/mob/living/carbon/human/update_inv_wrists()
	if(!HAS_TRAIT(src, TRAIT_HUMAN_NO_RENDER))
		...
		if(wrists)
			...
			overlays_standing[WRISTS_LAYER] = wrists.build_worn_icon(default_layer = WRISTS_LAYER, default_icon_file = 'modular_lumos/icons/mob/clothing/wrists.dmi')
...
/mob/living/carbon/human/update_inv_ears()
	if(!HAS_TRAIT(src, TRAIT_HUMAN_NO_RENDER))
		...
		if(ears)
			...
			overlays_standing[EAR_LEFT_LAYER] = ears.build_worn_icon(default_layer = EAR_LEFT_LAYER, default_icon_file = 'modular_lumos/icons/mob/clothing/ears.dmi', use_mob_overlay_icon = FALSE)
...
/mob/living/carbon/human/update_inv_ears_extra()
	if(!HAS_TRAIT(src, TRAIT_HUMAN_NO_RENDER))
		...
		if(ears_extra)
			...
			overlays_standing[EAR_RIGHT_LAYER] = ears_extra.build_worn_icon(default_layer = EAR_RIGHT_LAYER, default_icon_file = 'modular_lumos/icons/mob/clothing/ears_extra.dmi', use_mob_overlay_icon = FALSE)
